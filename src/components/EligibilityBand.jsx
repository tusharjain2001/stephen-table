import SectionChip from './SectionChip.jsx';

/** Real copy from the Figma Services page (identical on Nominate a Senior). */
const DEFAULT_ITEMS = [
  'Be an older adult of 60 years or more residing in Colorado.',
  'Require assistance with everyday home or community-based needs.',
  'Be seeking practical support, companionship, or access to community resources.',
  'Complete a service request or be nominated by a family member, caregiver, or community member.',
  'Meet program availability and service area requirements.',
];

/**
 * Brown "Eligibility Criteria" band used on Services and Nominate a Senior
 * (Implementation Plan §3.1 "EligibilityBand"). Copy is identical on both
 * pages, so sensible defaults are provided; override via props if needed.
 *
 * Props:
 * - `intro`     — intro line under the chip
 * - `introSize` — px intro type size from `md` up (default 24). Services'
 *                 redrawn 363:106 runs it at 20; Nominate's frame has not
 *                 been re-fetched, so it keeps 24. The band is `min-h-511`
 *                 with its content centred and the intro is one line either
 *                 way, so this changes nothing but the type.
 * - `items`     — array of bullet strings
 * - `bulletSize`— px bullet type size from `xl` up (default 24, which is what
 *                 Services' 363:118 draws). **Nominate's 378:3222 runs the
 *                 same copy at 20**, and that changes the wrap: at 24 item 3
 *                 needs two lines on the 797 measure (7 lines / 259px), at 20
 *                 only item 4 does (6 lines / 222px). The band is
 *                 `min-h-511` with its content centred, so both fit and a
 *                 section-height check can never see the difference.
 * - `bgClassName` — background color class; defaults to `bg-b-500` (every
 *                 current caller's hardcoded value). The maroon rollout
 *                 passes `bg-m-700` on Services/Nominate.
 * - `minHeightClassName` — min-height utility for the band; defaults to
 *                 `md:min-h-[511px]` (the current hardcoded token). Nominate's
 *                 maroon redesign restructures the band with a bullet list +
 *                 centered CTA, whose taller content sizes the band on its
 *                 own, so it passes `''` to drop the fixed floor.
 * - `cta`       — optional node (e.g. a copy line + Button) rendered as a
 *                 centered column (`flex w-full flex-col items-center
 *                 gap-[24px]`) below the bullet grid. When present, the gap
 *                 above it is 72px at md+; mobile keeps the existing 40px
 *                 stack rhythm. `null` (default) renders nothing extra — a
 *                 strict no-op for every current caller.
 * - `className` — extra classes on the outer <section>
 */
function EligibilityBand({
  intro = 'To be eligible for support, individuals should:',
  introSize = 24,
  items = DEFAULT_ITEMS,
  bulletSize = 24,
  bgClassName = 'bg-b-500',
  minHeightClassName = 'md:min-h-[511px]',
  cta = null,
  className = '',
}) {
  return (
    <section
      className={`flex w-full flex-col justify-center px-[16px] py-[60px] md:px-10 md:py-12 lg:px-12 xl:px-[72px] xl:py-[64px] ${bgClassName} ${minHeightClassName} ${className}`}
    >
      <div className="mx-auto flex w-full max-w-[1296px] flex-col gap-[40px] md:gap-[72px]">
        {/* Two-column grid (Figma 363:151): the chip sits in a max-content
            column and the intro line + bullet list share the second column, so
            the bullets stay flush with the intro instead of running back to the
            section's left edge. Below md it collapses to a single stack — the
            mobile frame (662:9459) groups chip+intro with their own 24px gap
            (via a `contents`-at-md wrapper so the md+ grid positioning below is
            untouched) and widens the gap to the list to 40px. */}
        <div className="grid w-full gap-y-[40px] md:grid-cols-[max-content_1fr] md:items-center md:gap-x-10 md:gap-y-[27px] xl:gap-x-16 2xl:gap-x-[226px]">
          <div className="flex flex-col gap-[24px] md:contents">
            <SectionChip
              variant="onBrown"
              // `self-start` is the base-tier fix: below md the wrapper is a
              // flex column, whose default `stretch` blew the chip out to the
              // full band width — `justify-self-start` does nothing in flex.
              // `md:self-auto` hands alignment back to the grid's `items-center`
              // from md up, so the desktop chip is unchanged.
              className="self-start justify-self-start md:self-auto md:col-start-1 md:row-start-1"
            >
              Eligibility Criteria
            </SectionChip>
            <p
              className="min-w-0 font-sans text-[16px] text-white md:col-start-2 md:row-start-1 md:text-[length:var(--eb-intro-size)] 2xl:w-[819px]"
              style={{ '--eb-intro-size': `${introSize}px` }}
            >
              {intro}
            </p>
          </div>

          {/* Figma runs the bullets as one continuous 37px-leading block at
              md+ — no extra spacing between items. The 797px measure
              (363:118) is load bearing: it wraps items 3 and 4 onto two
              lines, so the block is 7 lines / 259px, not 5 lines / 185px.
              Left unbounded it stops wrapping above ~1440 and the band goes
              hollow. Mobile instead stacks items with an explicit 19px gap. */}
          <ul className="flex min-w-0 flex-col gap-[19px] md:block md:col-start-2 md:row-start-2">
            {items.map((item) => (
              <li key={item} className="flex items-start gap-[12.5px] md:gap-[16px] xl:gap-[15px]">
                <span className="mt-[3px] size-[10px] shrink-0 rotate-45 bg-white md:mt-[11px] xl:mt-[13px]" />
                <span
                  className="font-sans text-[16px] leading-[17px] text-white md:text-[20px] md:leading-[28px] xl:text-[length:var(--eb-bullet-size)] xl:leading-[37px] 2xl:w-[797px]"
                  style={{ '--eb-bullet-size': `${bulletSize}px` }}
                >
                  {item}
                </span>
              </li>
            ))}
          </ul>
        </div>

        {cta && <div className="flex w-full flex-col items-center gap-[24px]">{cta}</div>}
      </div>
    </section>
  );
}

export default EligibilityBand;
